...
print("="*60)
print("B.TECH PROJECT: COMPREHENSIVE 3-MODEL EVALUATION (IN22-GEN BF16)")
print("="*60)
...
# ==========================================
# 2. HARDCODED PARAM-1 RESULTS & MODEL CONFIGS
# ==========================================
# Hardcoding Param-1 results from your previous run so we don't wait 1.5 hours
results = {
    "Param-1": {
        "BLEU": 0.74,
        "chrF": 6.21,
...
# Loading only the pure BF16 adapters for Pragna and Sarvam
models_to_test = [
    {
        "name": "Pragna-1B",
        "base_id": "soketlabs/pragna-1b",
        "adapter_id": "hdahiya/pragna-1b-hindi-transladtor-bf16", 
        "trust_remote_code": False
    },
    {
        "name": "Sarvam-1",
        "base_id": "sarvamai/sarvam-1",
        "adapter_id": "hdahiya/sarvam-1-hindi-translator-bf16", 
        "trust_remote_code": False
    }
]
...
for config in models_to_test:
    ...
    print(f"\n" + "="*40)
    print(f" EVALUATING: {model_name} (Native BF16)")
    print("="*40)
    ...
    # 3B. Attach Your BF16 Optimal Weights (LoRA)
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs, 
                ...
                # Removed use_cache=False to maximize generation speed!
            )
...
report_path = "./BTech_Comparative_Report_IN22_BF16.txt"
report_content = f"""==================================================
B.TECH PROJECT: FINAL ARCHITECTURE COMPARISON (IN22-GEN BF16)
==================================================
...
FINAL QUANTITATIVE METRICS (Evaluated in True BF16):
--------------------------------------------------
"""
...
api.upload_file(
    ...
    path_in_repo="BTech_Comparative_Report_IN22_BF16.txt",
    repo_id=hf_report_repo,
    repo_type="dataset",
    token=HF_TOKEN
)

print(f"\nSUCCESS! True BF16 metrics for IN22 have been saved and uploaded.")
